...
(pessoa.andar = (valor) => (pessoa.caminhouQuantosMetros += valor)),
  (pessoa.andando = true);
...
pessoa.apresentacao = () => {
  ...
  let idadeAno = `tenho ${pessoa.idade} ${pessoa.idade == 1 ? 'ano' : 'anos'}`;

  var altura = `${pessoa.caminhouQuantosMetros} ${
    pessoa.caminhouQuantosMetros == 1 ? 'metro' : 'metros'
  }`;

  return `${intro}, ${idadeAno} , ${pessoa.altura}, meu peso é ${pessoa.peso} e, só hoje, eu já caminhei ${altura} !`;
};
